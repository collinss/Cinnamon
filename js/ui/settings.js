...
XletSettingsBase.prototype = {
    ...
    bindProperty: function(direction, key, applet_prop, callback, user_data) {
        if (!this.isReady) {
            settings_not_initialized_error(uuid);
            return false;
        }
        if (!(key in this.settingsData)){
            key_not_found_error(key, uuid);
            return false;
        }
        if (!(this.settingsData[key].type in SETTINGS_TYPES)) {
            invalid_setting_type_error(key, uuid, this.settingsData[key].type);
            return false;
        }
        ...
        if(typeof(this.settingsData[key]) === "object" && !this.settingsData[key].save){
            this.settingsData[key].save = Lang.bind(this, this._saveToFile);
        }
    ...
    _setValue: function(value, key) {
        if (this.settingsData[key].value != value) {
    ...
    _doUpgrade: function(templateData, checksum) {
        ...
        for (let key in newSettings) {
            ...
            if (key in this.settingsData && this._checkSanity(oldValue)) newSettings[key].value = oldValue;
