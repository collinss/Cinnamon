...
class Module:
    ...
    def on_module_selected(self):
        ...
        widget = g_settings_factory(Switch, _("Lock the computer when put to sleep"), "org.cinnamon.settings-daemon.plugins.power", "lock-on-suspend")
        widget.set_tooltip_text(_("Enable this option to require a password when the computer wakes up from suspend"))
        settings.add_row(widget)

        widget = g_settings_factory(Switch, _("Lock the computer when the screen turns off"), schema, "lock-enabled")
        ...
        widget = g_settings_factory(ComboBox, _("Delay before locking the screen"), schema, "lock-delay", LOCK_DELAY_OPTIONS, valtype="uint", size_group=size_group)
        ...
        widget = g_settings_factory(ComboBox, _("Lock the computer when inactive"), "org.cinnamon.desktop.session", "idle-delay", LOCK_INACTIVE_OPTIONS, valtype="uint", size_group=size_group)
        ...
        widget = g_settings_factory(Entry, _("Show this message when the screen is locked"), schema, "default-message")
        ...
        settings.add_row(g_settings_factory(FontButton, _("Font"), "org.cinnamon.desktop.screensaver", "font-message"))

        widget = g_settings_factory(Switch, _("Ask for a custom message when locking the screen from the menu"), schema, "ask-for-away-message")
        ...
        widget = g_settings_factory(Switch, _("Use a custom date and time format"), schema, "use-custom-format")
        settings.add_row(widget)

        widget = g_settings_factory(Entry, _("Time Format"), schema, "time-format", size_group=size_group)
        settings.add_reveal_row(widget, schema, "use-custom-format")

        widget = g_settings_factory(Entry, _("Date Format: "), schema, "date-format", size_group=size_group)
        settings.add_reveal_row(widget, schema, "use-custom-format")

        widget = g_settings_factory(FontButton, _("Time Font"), "org.cinnamon.desktop.screensaver", "font-time", size_group=size_group)
        settings.add_row(widget)

        widget = g_settings_factory(FontButton, _("Date Font"), "org.cinnamon.desktop.screensaver", "font-date", size_group=size_group)
        settings.add_row(widget)
